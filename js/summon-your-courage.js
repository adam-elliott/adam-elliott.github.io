...
$(document).ready(function() {
	...
	function loadItems(level) {
		for (key in level.goals) {
			if (level.goals[key].isFound != true) {
				...
				myCell.style.left = level.goals[key].x + "px";
				myCell.style.top = level.goals[key].y + "px";
	...
	function handleModifiers() {
		for (key in level.map) {
			let iter = key;
			if (level.map[key].item == "modifier") {
				for (key in level.map[iter].cells) {
					if (level.map[iter].cells[key].x == curPosX && level.map[iter].cells[key].y == curPosY) {
